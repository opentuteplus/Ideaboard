...
class BBP_Forums_Group_Extension extends BP_Group_Extension {
	...
	private function setup_actions() {
		...
		// Possibly redirect
		add_action( 'ideaboard_template_redirect',         array( $this, 'redirect_canonical'              ) );

		// Remove group forum cap map when view is done
		add_action( 'ideaboard_after_group_forum_display', array( $this, 'remove_group_forum_meta_cap_map' ) );
	...
	private function setup_filters() {
		...
		add_filter( 'ideaboard_topic_pagination',      array( $this, 'topic_pagination'   ) );
		add_filter( 'ideaboard_replies_pagination',    array( $this, 'replies_pagination' ) );

		// Tweak the redirect field
		add_filter( 'ideaboard_new_topic_redirect_to', array( $this, 'new_topic_redirect_to'        ), 10, 3 );
		add_filter( 'ideaboard_new_reply_redirect_to', array( $this, 'new_reply_redirect_to'        ), 10, 3 );

		// Map forum/topic/replys permalinks to their groups
		add_filter( 'ideaboard_get_forum_permalink',   array( $this, 'map_forum_permalink_to_group' ), 10, 2 );
		add_filter( 'ideaboard_get_topic_permalink',   array( $this, 'map_topic_permalink_to_group' ), 10, 2 );
		add_filter( 'ideaboard_get_reply_permalink',   array( $this, 'map_reply_permalink_to_group' ), 10, 2 );

		// Map reply edit links to their groups
		add_filter( 'ideaboard_get_reply_edit_url',    array( $this, 'map_reply_edit_url_to_group'  ), 10, 2 );
		...
		add_filter( 'ideaboard_before_record_activity_parse_args', array( $this, 'map_activity_to_group' ) );
		...
		if ( bp_is_single_item() && bp_is_groups_component() && bp_is_current_action( 'forum' ) ) {
			...
			add_filter( 'ideaboard_map_meta_caps', array( $this, 'map_group_forum_meta_caps' ), 10, 4 );

			// Group member permissions to view the topic and reply forms
			add_filter( 'ideaboard_current_user_can_access_create_topic_form', array( $this, 'form_permissions' ) );
			add_filter( 'ideaboard_current_user_can_access_create_reply_form', array( $this, 'form_permissions' ) );
		}
	}
	...
	public function display( $group_id = null ) {
		...
		add_action( 'ideaboard_theme_before_topic_form_forum', array( $this, 'ob_start'     ) );
		add_action( 'ideaboard_theme_after_topic_form_forum',  array( $this, 'ob_end_clean' ) );
		add_action( 'ideaboard_theme_after_topic_form_forum',  array( $this, 'topic_parent' ) );

		// Prevent Forum Parent from appearing
		add_action( 'ideaboard_theme_before_forum_form_parent', array( $this, 'ob_start'     ) );
		add_action( 'ideaboard_theme_after_forum_form_parent',  array( $this, 'ob_end_clean' ) );
		add_action( 'ideaboard_theme_after_forum_form_parent',  array( $this, 'forum_parent' ) );

		// Hide breadcrumb
		add_filter( 'ideaboard_no_breadcrumb', '__return_true' );
	...
	public function map_group_forum_meta_caps( $caps = array(), $cap = '', $user_id = 0, $args = array() ) {
		...
		switch ( $cap ) {
			...
			case 'read_private_forums' :
				if ( ideaboard_group_is_member() || ideaboard_group_is_mod() || ideaboard_group_is_admin() ) {
			...
			case 'edit_others_topics'  :
				if ( ideaboard_group_is_mod() || ideaboard_group_is_admin() ) {
					$caps = array( 'participate' );
				}
				break;
			...
			case 'delete_topic' :
			case 'delete_reply' :
				if ( ideaboard_group_is_admin() ) {
					$caps = array( 'participate' );
				}
				break;
		}

		return apply_filters( 'ideaboard_map_group_forum_topic_meta_caps', $caps, $cap, $user_id, $args );
	}
	...
	public function remove_group_forum_meta_cap_map() {
		remove_filter( 'ideaboard_map_meta_caps', array( $this, 'map_group_forum_meta_caps' ), 99, 4 );
	}
	...
	public function edit_screen( $group = false ) {
		...
		<fieldset>
			...
			<?php if ( ideaboard_is_user_keymaster() ) : ?>
				<div class="field-group">
					<label for="ideaboard_group_forum_id"><?php esc_html_e( 'Group Forum:', 'ideaboard' ); ?></label>
					<?php
						ideaboard_dropdown( array(
							'select_id' => 'ideaboard_group_forum_id',
	 ...
	 * @uses ideaboard_new_forum_handler() To check for forum creation
	 * @uses ideaboard_edit_forum_handler() To check for forum edit
	 */
	public function edit_screen_save( $group_id = 0 ) {
		...
		} elseif ( ! ideaboard_verify_nonce_request( 'groups_edit_save_' . $this->slug ) ) {
			ideaboard_add_error( 'ideaboard_edit_group_forum_screen_save', __( '<strong>ERROR</strong>: Are you sure you wanted to do that?', 'ideaboard' ) );
			return;
 		}
		...
		if ( ideaboard_is_user_keymaster() ) {
			$forum_ids = ! empty( $_POST['ideaboard_group_forum_id'] ) ? (array) (int) $_POST['ideaboard_group_forum_id'] : array();

		// Use the existing forum IDs
		} else {
			$forum_ids = array_values( ideaboard_get_group_forum_ids( $group_id ) );
		}
		...
		ideaboard_update_group_forum_ids( $group_id, (array) $forum_ids );
		ideaboard_update_forum_group_ids( $forum_id, (array) $group_id  );
		...
		if ( empty( $forum_id ) && ( true === $edit_forum ) ) {
			...
			switch ( $group->status ) {
				case 'hidden'  :
					$status = ideaboard_get_hidden_status_id();
					break;
				case 'private' :
					$status = ideaboard_get_private_status_id();
					break;
				case 'public'  :
				default        :
					$status = ideaboard_get_public_status_id();
					break;
			}

			// Create the initial forum
			$forum_id = ideaboard_insert_forum( array(
				'post_parent'  => ideaboard_get_group_forums_root_id(),
	...
	public function create_screen_save( $group_id = 0 ) {
		...
		// Nonce check
		if ( ! ideaboard_verify_nonce_request( 'groups_create_save_' . $this->slug ) ) {
			ideaboard_add_error( 'ideaboard_create_group_forum_screen_save', __( '<strong>ERROR</strong>: Are you sure you wanted to do that?', 'ideaboard' ) );
			return;
		}
		...
		switch ( $create_forum ) {
			case true  :
				...
				// Update forum active
				groups_update_groupmeta( bp_get_new_group_id(), '_ideaboard_forum_enabled_' . $forum_id, true );
			...
			case false :
				...
				if ( !empty( $forum_id ) ) {
					...
					groups_delete_groupmeta( bp_get_new_group_id(), '_ideaboard_forum_enabled_' . $forum_id );
	 ...
	 * @uses ideaboard_add_forum_id_to_group()
	 * @uses ideaboard_add_group_id_to_forum()
	...
	public function new_forum( $forum_args = array() ) {
		...
		// Validate forum_id
		$forum_id = ideaboard_get_forum_id( $forum_args['forum_id'] );
		$group_id = !empty( $forum_args['group_id'] ) ? $forum_args['group_id'] : bp_get_current_group_id();

		ideaboard_add_forum_id_to_group( $group_id, $forum_id );
		ideaboard_add_group_id_to_forum( $forum_id, $group_id );
	}
	...
	public function remove_forum( $forum_args = array() ) {
		...
		ideaboard_remove_forum_id_from_group( $group_id, $forum_id );
		ideaboard_remove_group_id_from_forum( $forum_id, $group_id );
	}
	...
	public function toggle_group_forum( $group_id = 0, $enabled = false ) {
		...
		ideaboard_repair_forum_visibility();
	...
	public function display_forums( $offset = 0 ) {
		...
		$forum_ids    = ideaboard_get_group_forum_ids( bp_get_current_group_id() );
		$forum_id     = array_shift( $forum_ids );

		// Always load up the group forum
		ideaboard_has_forums( array(
			'p'           => $forum_id,
			'post_parent' => null
		) );
		...
		<div id="ideaboard-forums">
			...
			<?php switch ( $forum_action ) :
				...
				case 'page' :
					...
					add_filter( 'ideaboard_get_super_stickies', array( $this, 'no_super_stickies'  ), 10, 1 );

					// Unset the super sticky option on topic form
					add_filter( 'ideaboard_get_topic_types',    array( $this, 'unset_super_sticky' ), 10, 1 );
					...
					if ( ideaboard_forums() ) :
						...
						<h3><?php ideaboard_forum_title(); ?></h3>
				...
				case $this->topic_slug :
					...
					add_filter( 'ideaboard_get_topic_stick_link', array( $this, 'hide_super_sticky_admin_link' ), 10, 2 );

					// Get the topic
					ideaboard_has_topics( array(
					...
					// If no topic, 404
					if ( ! ideaboard_topics() ) {
						bp_do_404( ideaboard_get_forum_permalink( $forum_id ) ); ?>
						<h3><?php ideaboard_forum_title(); ?></h3>
					...
					<h3><?php ideaboard_topic_title(); ?></h3>

					<?php

					// Topic edit
					if ( bp_action_variable( $offset + 2 ) === ideaboard_get_edit_rewrite_id() ) :

						// Unset the super sticky link on edit topic template
						add_filter( 'ideaboard_get_topic_types', array( $this, 'unset_super_sticky' ), 10, 1 );

						// Set the edit switches
						$wp_query->ideaboard_is_edit       = true;
						...
						if ( !empty( $_GET['action'] ) && 'merge' === $_GET['action'] ) :
							ideaboard_set_query_name( 'ideaboard_topic_merge' );
							ideaboard_get_template_part( 'form', 'topic-merge' );

						// Split
						elseif ( !empty( $_GET['action'] ) && 'split' === $_GET['action'] ) :
							ideaboard_set_query_name( 'ideaboard_topic_split' );
							ideaboard_get_template_part( 'form', 'topic-split' );

						// Edit
						else :
							ideaboard_set_query_name( 'ideaboard_topic_form' );
					...
					// Single Topic
						...
						ideaboard_set_query_name( 'ideaboard_single_topic' );
				...
				case $this->reply_slug :
					...
					// Get the reply
					ideaboard_has_replies( array(
						'name'           => bp_action_variable( $offset + 1 ),
						'posts_per_page' => 1
					) );

					// If no topic, 404
					if ( ! ideaboard_replies() ) {
						bp_do_404( ideaboard_get_forum_permalink( $forum_id ) ); ?>
						<h3><?php ideaboard_forum_title(); ?></h3>
						<?php ideaboard_get_template_part( 'feedback', 'no-replies' );
						return;
					}
					...
					<h3><?php ideaboard_reply_title(); ?></h3>

					<?php if ( bp_action_variable( $offset + 2 ) === ideaboard_get_edit_rewrite_id() ) :
						...
						if ( !empty( $_GET['action'] ) && ( 'move' === $_GET['action'] ) ) :
							ideaboard_set_query_name( 'ideaboard_reply_move' );
							ideaboard_get_template_part( 'form', 'reply-move' );

						// Edit
						else :
							ideaboard_set_query_name( 'ideaboard_reply_form' );
		...
		do_action( 'ideaboard_after_group_forum_display' );
	}
	...
	public function new_reply_redirect_to( $redirect_url = '', $redirect_to = '', $reply_id = 0 ) {
		...
		if ( bp_is_group() ) {
			...
			$topic          = ideaboard_get_topic( $topic_id );
			$reply_position = ideaboard_get_reply_position( $reply_id, $topic_id );
			$reply_page     = ceil( (int) $reply_position / (int) ideaboard_get_replies_per_page() );
	...
	?>
		...
		<input type="hidden" name="ideaboard_forum_parent_id" id="ideaboard_forum_parent_id" value="<?php ideaboard_group_forums_root_id(); ?>" />
	...
	public function topic_parent() {
		...
		<p>
			<label for="ideaboard_forum_id"><?php esc_html_e( 'Forum:', 'ideaboard' ); ?></label><br />
			<?php ideaboard_dropdown( array( 'include' => $forum_ids, 'selected' => ideaboard_get_form_topic_forum() ) ); ?>
		</p>
	 ...
	 * @uses ideaboard_is_user_keymaster() Is the current user a keymaster?
	 * @uses ideaboard_group_is_member() Is the current user a member of the group?
	 * @uses ideaboard_group_is_user_banned() Is the current user banned from the group?
	 *
	 * @return bool
	 */
	public function form_permissions( $retval = false ) {
		...
		} elseif ( ideaboard_is_user_keymaster() ) {
			$retval = true;

		// Non-members cannot see forms
		} elseif ( ! ideaboard_group_is_member() ) {
			$retval = false;

		// Banned users cannot see forms
		} elseif ( ideaboard_group_is_banned() ) {
			$retval = false;
		}
	 ...
	 * @param int $post_id
	 * @uses get_post()
	 * @uses ideaboard_is_reply()
	 * @uses ideaboard_get_reply_topic_id()
	 * @uses ideaboard_get_reply_forum_id()
	 * @uses ideaboard_is_topic()
	 * @uses ideaboard_get_topic_forum_id()
	 * @uses ideaboard_is_forum()
	 * @uses get_post_field()
	 * @uses ideaboard_get_forum_group_ids()
	...
	private function maybe_map_permalink_to_group( $post_id = 0, $url = false ) {
		...
		switch ( get_post_type( $post_id ) ) {
			...
			case ideaboard_get_reply_post_type() :
				$topic_id = ideaboard_get_reply_topic_id( $post_id );
				$forum_id = ideaboard_get_reply_forum_id( $post_id );
			...
			case ideaboard_get_topic_post_type() :
				$topic_id = $post_id;
				$forum_id = ideaboard_get_topic_forum_id( $post_id );
	...
	public function map_reply_permalink_to_group( $url, $reply_id ) {
		return $this->maybe_map_permalink_to_group( ideaboard_get_reply_topic_id( $reply_id ), $url );
	}
	...
	public function topic_pagination( $args ) {
		$new = $this->maybe_map_permalink_to_group( ideaboard_get_forum_id() );
	...
	public function replies_pagination( $args ) {
		$new = $this->maybe_map_permalink_to_group( ideaboard_get_topic_id() );
		if ( empty( $new ) )
			return $args;
