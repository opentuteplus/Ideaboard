...
class IdeaBoard_Akismet {
	...
	private function setup_actions() {
		...
		$checks['check']  = array(
			'ideaboard_new_topic_pre_insert'  => 1,  // New topic check
			'ideaboard_new_reply_pre_insert'  => 1,  // New reply check
			'ideaboard_edit_topic_pre_insert' => 1,  // Edit topic check
			'ideaboard_edit_reply_pre_insert' => 1   // Edit reply check
		);

		// IdeaBoard functions for spam and ham submissions
		$checks['submit'] = array(
			'ideaboard_spammed_topic'   => 10, // Spammed topic
			'ideaboard_unspammed_topic' => 10, // Unspammed reply
			'ideaboard_spammed_reply'   => 10, // Spammed reply
			'ideaboard_unspammed_reply' => 10, // Unspammed reply
		);
	...
	public function check_post( $post_data ) {
		...
		if ( ideaboard_get_public_status_id() !== $post_data['post_status'] )
			return $post_data;
		...
		if ( !empty( $anonymous_data ) ) {
			$user_data['name']    = $anonymous_data['ideaboard_anonymous_name'];
			$user_data['email']   = $anonymous_data['ideaboard_anonymous_email'];
			$user_data['website'] = $anonymous_data['ideaboard_anonymous_website'];
		...
		$_post = array(
			...
			'user_ip'              => ideaboard_current_author_ip(),
		...
		// Get the result
		$post_data['ideaboard_akismet_result'] = $_post['ideaboard_akismet_result'];
		unset( $_post['ideaboard_akismet_result'] );

		// Store the data as submitted
		$post_data['ideaboard_post_as_submitted'] = $_post;

		// Allow post_data to be manipulated
		do_action_ref_array( 'ideaboard_akismet_check_post', $post_data );

		// Spam
		if ( 'true' === $post_data['ideaboard_akismet_result'] ) {

			// Let plugins do their thing
			do_action( 'ideaboard_akismet_spam_caught' );

			// This is spam
			$post_data['post_status'] = ideaboard_get_spam_status_id();

			// We don't want your spam tags here
			add_filter( 'ideaboard_new_reply_pre_set_terms', array( $this, 'filter_post_terms' ), 1, 3 );
	...
	public function submit_post( $post_id = 0 ) {
		...
		switch ( $current_filter ) {
			...
			case 'ideaboard_spammed_topic' :
			case 'ideaboard_spammed_reply' :
				$request_type = 'spam';
				break;

			// Honey-glazed, a straight off the bone
			case 'ideaboard_unspammed_topic' :
			case 'ideaboard_unspammed_reply' :
				$request_type = 'ham';
				break;
		...
		if ( ( 'spam' === $request_type ) && ( ideaboard_get_spam_status_id() !== $_post->post_status ) )
			return;

		// Set some default post_data
		$post_data = array(
			'comment_approved'     => $_post->post_status,
			'comment_author'       => $_post->post_author ? get_the_author_meta( 'display_name', $_post->post_author ) : get_post_meta( $post_id, '_ideaboard_anonymous_name',    true ),
			'comment_author_email' => $_post->post_author ? get_the_author_meta( 'email',        $_post->post_author ) : get_post_meta( $post_id, '_ideaboard_anonymous_email',   true ),
			'comment_author_url'   => $_post->post_author ? ideaboard_get_user_profile_url(            $_post->post_author ) : get_post_meta( $post_id, '_ideaboard_anonymous_website', true ),
		...
		if ( isset( $post_data['reporter'] ) ) {
			...
			switch ( $request_type ) {
				...
				case 'spam' :
					...
					update_post_meta( $post_id, '_ideaboard_akismet_user_result', 'true'                 );
					update_post_meta( $post_id, '_ideaboard_akismet_user',        $post_data['reporter'] );
					break;
				...
				case 'ham'  :
					...
					update_post_meta( $post_id, '_ideaboard_akismet_user_result', 'false'                 );
					update_post_meta( $post_id, '_ideaboard_akismet_user',         $post_data['reporter'] );
		...
		do_action( 'ideaboard_akismet_submit_' . $request_type . '_post', $post_id, $post_data['ideaboard_akismet_result'] );
	}
	...
	private function maybe_spam( $post_data, $check = 'check', $spam = 'spam' ) {
		...
		if ( !empty( $response[1] ) ) {
			$post_data['ideaboard_akismet_result'] = $response[1];
		} else {
			$post_data['ideaboard_akismet_result'] = esc_html__( 'No response', 'ideaboard' );
		}
	...
	public function update_post_meta( $post_id = 0, $_post = false ) {
		...
		if ( is_object( $_post ) && !empty( $this->last_post ) && is_array( $as_submitted ) ) {
			...
			if (	intval( $as_submitted['comment_post_ID'] )    === intval( $_post->post_parent )
					&&      $as_submitted['comment_author']       === ( $anonymous_data ? $anonymous_data['ideaboard_anonymous_name']  : $userdata->display_name )
					&&      $as_submitted['comment_author_email'] === ( $anonymous_data ? $anonymous_data['ideaboard_anonymous_email'] : $userdata->user_email   )
				) {

				// Normal result: true
				if ( $this->last_post['ideaboard_akismet_result'] === 'true' ) {

					// Leave a trail so other's know what we did
					update_post_meta( $post_id, '_ideaboard_akismet_result', 'true' );
					...
					if ( ideaboard_get_spam_status_id() !== $_post->post_status ) {
				...
				// Normal result: false
				} elseif ( $this->last_post['ideaboard_akismet_result'] === 'false' ) {

					// Leave a trail so other's know what we did
					update_post_meta( $post_id, '_ideaboard_akismet_result', 'false' );
				...
				// Abnormal result: error
					...
					// Leave a trail so other's know what we did
					update_post_meta( $post_id, '_ideaboard_akismet_error', time() );
					$this->update_post_history( $post_id, sprintf( esc_html__( 'Akismet was unable to check this post (response: %s), will automatically retry again later.', 'ideaboard' ), $this->last_post['ideaboard_akismet_result'] ), 'check-error' );
				}

				// Record the complete original data as submitted for checking
				if ( isset( $this->last_post['ideaboard_post_as_submitted'] ) ) {
					update_post_meta( $post_id, '_ideaboard_akismet_as_submitted', $this->last_post['ideaboard_post_as_submitted'] );
				}
			}
		}
	}
	...
	private function update_post_history( $post_id = 0, $message = null, $event = null ) {
		...
		// Save the event data
		add_post_meta( $post_id, '_ideaboard_akismet_history', $event );
	}
	...
	public function get_post_history( $post_id = 0 ) {
		...
		$history = get_post_meta( $post_id, '_ideaboard_akismet_history' );
	 ...
	 * @param int $reply_id
	 *
	 * @uses ideaboard_get_reply_id() To get the reply_id
	 * @uses ideaboard_get_topic_id() To get the topic_id
	...
	public function filter_post_terms( $terms = '', $topic_id = 0, $reply_id = 0 ) {
		...
		$existing_terms = wp_get_object_terms( $topic_id, ideaboard_get_topic_tag_tax_id(), array( 'fields' => 'names' ) );

		// Save the terms for later in case the reply gets hammed
		if ( !empty( $terms ) )
			update_post_meta( $reply_id, '_ideaboard_akismet_spam_terms', $terms );
	...
	private function http_post( $request, $host, $path, $port = 80, $ip = '' ) {
		...
		$ideaboard_version    = ideaboard_get_version();
		...
		// Untque User Agent
		$akismet_ua     = "IdeaBoard/{$ideaboard_version} | ";
	...
	public function add_metaboxes() {
		...
		// Topics
		add_meta_box(
			'ideaboard_akismet_topic_history',
		...
		// Replies
		add_meta_box(
			'ideaboard_akismet_reply_history',
			__( 'Akismet History', 'ideaboard' ),
			array( $this, 'history_metabox' ),
	 ...
	 * @uses ideaboard_time_since() To get the human readable time
	 */
							...
							<tr>
								<td style="color: #999; text-align: right; white-space: nowrap;">
									<span title="<?php echo esc_attr( date( 'D d M Y @ h:i:m a', $row['time'] ) . ' GMT' ); ?>">
										<?php ideaboard_time_since( $row['time'], false, true ); ?>
									</span>
								</td>
