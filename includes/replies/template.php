 ...
 */
function ideaboard_reply_post_type() {
	echo ideaboard_get_reply_post_type();
}
	/**
	 * Return the unique id of the custom post type for replies
	 *
	 ...
	 */
	function ideaboard_get_reply_post_type() {
		return apply_filters( 'ideaboard_get_reply_post_type', ideaboard()->reply_post_type );
	}
 ...
 * @return array
 */
function ideaboard_get_reply_post_type_labels() {
	return apply_filters( 'ideaboard_get_reply_post_type_labels', array(
 ...
 * @return array
 */
function ideaboard_get_reply_post_type_rewrite() {
	return apply_filters( 'ideaboard_get_reply_post_type_rewrite', array(
		'slug'       => ideaboard_get_reply_slug(),
		'with_front' => false
	) );
}
 ...
 * @return array
 */
function ideaboard_get_reply_post_type_supports() {
	return apply_filters( 'ideaboard_get_reply_post_type_supports', array(
		'title',
		'editor',
		'revisions'
	) );
 ...
 * @uses ideaboard_show_lead_topic() Are we showing the topic as a lead?
 ...
 * @uses apply_filters() Calls 'ideaboard_replies_pagination' with the pagination args
 * @uses paginate_links() To paginate the links
 * @uses apply_filters() Calls 'ideaboard_has_replies' with
 *                        bbPres::reply_query::have_posts()
 *                        and bbPres::reply_query
 * @return object Multidimensional array of reply information
 */
function ideaboard_has_replies( $args = '' ) {
	global $wp_rewrite;
	...
	$default_post_parent    = ( ideaboard_is_single_topic() ) ? ideaboard_get_topic_id() : 'any';
	$default_post_type      = ( ideaboard_is_single_topic() && ideaboard_show_lead_topic() ) ? ideaboard_get_reply_post_type() : array( ideaboard_get_topic_post_type(), ideaboard_get_reply_post_type() );
	$default_thread_replies = (bool) ( ideaboard_is_single_topic() && ideaboard_thread_replies() );
	...
	$r = ideaboard_parse_args( $args, $default, 'has_replies' );
	...
	if ( (int) $ideaboard->reply_query->found_posts ) {
		...
		foreach ( $ideaboard->reply_query->posts as &$post ) {
			...
			if ( ideaboard_get_reply_post_type() === $post->post_type ) {
				$reply_to = ideaboard_get_reply_to( $post->ID );

				// Make sure it's a reply to a reply
				if ( empty( $reply_to ) || ( ideaboard_get_reply_topic_id( $post->ID ) === $reply_to ) ) {
					$reply_to = 0;
				}
	...
	if ( (int) $ideaboard->reply_query->found_posts && (int) $ideaboard->reply_query->posts_per_page ) {
		...
		if ( $wp_rewrite->using_permalinks() ) {
			...
			// User's replies
			if ( ideaboard_is_single_user_replies() ) {
				$base = ideaboard_get_user_replies_created_url( ideaboard_get_displayed_user_id() );
			...
			// Single topic
				...
				$base = get_permalink( ideaboard_get_topic_id() );
			}
 ...
 */
function ideaboard_reply_id( $reply_id = 0 ) {
	echo ideaboard_get_reply_id( $reply_id );
}
	/**
	 ...
	 * @uses ideaboard_is_reply() To check if the search result is a reply
	 * @uses ideaboard_is_single_reply() To check if it's a reply page
	 ...
	 * @uses ideaboard_get_reply_post_type() To get the reply post type
	 * @uses apply_filters() Calls 'ideaboard_get_reply_id' with the reply id and
	 *                        supplied reply id
	...
	function ideaboard_get_reply_id( $reply_id = 0 ) {
		...
		// Currently viewing a reply
			...
			$ideaboard_reply_id = $wp_query->post->ID;

		// Fallback
		} else {
			$ideaboard_reply_id = 0;
		}

		return (int) apply_filters( 'ideaboard_get_reply_id', $ideaboard_reply_id, $reply_id );
	}
 ...
 * @uses ideaboard_get_reply_post_type() To get the reply post type
 * @uses apply_filters() Calls 'ideaboard_get_reply' with the reply, output type and
 *                        sanitation filter
 * @return mixed Null if error or reply (in specified form) if success
 */
function ideaboard_get_reply( $reply, $output = OBJECT, $filter = 'raw' ) {
	if ( empty( $reply ) || is_numeric( $reply ) )
		$reply = ideaboard_get_reply_id( $reply );

	$reply = get_post( $reply, OBJECT, $filter );
	if ( empty( $reply ) )
		return $reply;

	if ( $reply->post_type !== ideaboard_get_reply_post_type() )
		return null;
	...
	return apply_filters( 'ideaboard_get_reply', $reply, $output, $filter );
}
 ...
 */
function ideaboard_reply_permalink( $reply_id = 0 ) {
	echo esc_url( ideaboard_get_reply_permalink( $reply_id ) );
}
	/**
	 ...
	 * @uses apply_filters() Calls 'ideaboard_get_reply_permalink' with the link
	 *                        and reply id
	 * @return string Permanent link to reply
	 */
	function ideaboard_get_reply_permalink( $reply_id = 0 ) {
		$reply_id = ideaboard_get_reply_id( $reply_id );

		return apply_filters( 'ideaboard_get_reply_permalink', get_permalink( $reply_id ), $reply_id );
	}
/**
 ...
 */
function ideaboard_reply_url( $reply_id = 0 ) {
	echo esc_url( ideaboard_get_reply_url( $reply_id ) );
}
	/**
	 ...
	 * @uses ideaboard_get_reply_position() To get the reply position
	 * @uses get_option() To get the replies per page option
	 * @uses WP_Rewrite::using_permalinks() To check if the blog uses
	 *                                       permalinks
	 * @uses add_query_arg() To add custom args to the url
	 * @uses apply_filters() Calls 'ideaboard_get_reply_url' with the reply url,
	 *                        reply id and bool count hidden
	 * @return string Link to reply relative to paginated topic
	 */
	function ideaboard_get_reply_url( $reply_id = 0, $redirect_to = '' ) {
		...
		// Standard reply page
			...
			$reply_page = ceil( (int) ideaboard_get_reply_position( $reply_id, $topic_id ) / (int) ideaboard_get_replies_per_page() );
		}

		$reply_hash = '#post-' . $reply_id;
		$topic_link = ideaboard_get_topic_permalink( $topic_id, $redirect_to );
		...
		return apply_filters( 'ideaboard_get_reply_url', $url, $reply_id, $redirect_to );
	}
 ...
 */
function ideaboard_reply_title( $reply_id = 0 ) {
	echo ideaboard_get_reply_title( $reply_id );
}
	 ...
	 * @uses apply_filters() Calls 'ideaboard_get_reply_title' with the title and
	 *                        reply id
	 * @return string Title of reply
	 */
	function ideaboard_get_reply_title( $reply_id = 0 ) {
		$reply_id = ideaboard_get_reply_id( $reply_id );

		return apply_filters( 'ideaboard_get_reply_title', get_the_title( $reply_id ), $reply_id );
	}
	 ...
	 * @uses apply_filters() Calls 'ideaboard_get_reply_title_fallback' with the title and reply ID
	 * @return string Title of reply
	 */
	function ideaboard_get_reply_title_fallback( $post_title = '', $post_id = 0 ) {

		// Bail if title not empty, or post is not a reply
		if ( ! empty( $post_title ) || ! ideaboard_is_reply( $post_id ) ) {
			return $post_title;
		}

		// Get reply topic title.
		$topic_title = ideaboard_get_reply_topic_title( $post_id );

		// Get empty reply title fallback.
		$reply_title = sprintf( __( 'Reply To: %s', 'ideaboard' ), $topic_title );

		return apply_filters( 'ideaboard_get_reply_title_fallback', $reply_title, $post_id, $topic_title );
	}
 ...
 *
 * @param int $reply_id Optional. reply id
 * @uses ideaboard_get_reply_content() To get the reply content
 */
function ideaboard_reply_content( $reply_id = 0 ) {
	echo ideaboard_get_reply_content( $reply_id );
}
	/**
	 ...
	 * @uses apply_filters() Calls 'ideaboard_get_reply_content' with the content
	 *                        and reply id
	 * @return string Content of the reply
	 */
	function ideaboard_get_reply_content( $reply_id = 0 ) {
		$reply_id = ideaboard_get_reply_id( $reply_id );
		...
		return apply_filters( 'ideaboard_get_reply_content', $content, $reply_id );
	}
 ...
 */
function ideaboard_reply_excerpt( $reply_id = 0, $length = 100 ) {
	echo ideaboard_get_reply_excerpt( $reply_id, $length );
}
	/**
	 ...
	 * @uses ideaboard_get_reply_content() To get the reply content
	 * @uses apply_filters() Calls 'ideaboard_get_reply_excerpt' with the excerpt,
	 *                        reply id and length
	 * @return string Reply Excerpt
	 */
	function ideaboard_get_reply_excerpt( $reply_id = 0, $length = 100 ) {
		$reply_id = ideaboard_get_reply_id( $reply_id );
		$length   = (int) $length;
		$excerpt  = get_post_field( 'post_excerpt', $reply_id );

		if ( empty( $excerpt ) ) {
			$excerpt = ideaboard_get_reply_content( $reply_id );
		}
		...
		return apply_filters( 'ideaboard_get_reply_excerpt', $excerpt, $reply_id, $length );
	}
 ...
 * @uses ideaboard_get_reply_post_date() to get the output
 */
function ideaboard_reply_post_date( $reply_id = 0, $humanize = false, $gmt = false ) {
	echo ideaboard_get_reply_post_date( $reply_id, $humanize, $gmt );
}
	/**
	 ...
	 * @uses ideaboard_get_time_since() to maybe humanize the reply post time
	 * @return string
	 */
	function ideaboard_get_reply_post_date( $reply_id = 0, $humanize = false, $gmt = false ) {
		$reply_id = ideaboard_get_reply_id( $reply_id );
		...
		return apply_filters( 'ideaboard_get_reply_post_date', $result, $reply_id, $humanize, $gmt, $date, $time );
	}
 ...
 * @param int $reply_id Optional. Reply id
 * @uses ideaboard_get_reply_revision_log() To get the reply revision log
 * @uses apply_filters() Calls 'ideaboard_reply_append_revisions' with the processed
 *                        content, original content and reply id
 * @return string Content with the revisions appended
 */
function ideaboard_reply_content_append_revisions( $content = '', $reply_id = 0 ) {
	...
	return apply_filters( 'ideaboard_reply_append_revisions', $content . ideaboard_get_reply_revision_log( $reply_id ), $content, $reply_id );
}
 ...
 * @uses ideaboard_get_reply_revision_log() To get the reply revision log
 */
function ideaboard_reply_revision_log( $reply_id = 0 ) {
	echo ideaboard_get_reply_revision_log( $reply_id );
}
	/**
	 ...
	 * @param int $reply_id Optional. Reply id
	 * @uses ideaboard_get_reply_id() To get the reply id
	 * @uses ideaboard_get_reply_revisions() To get the reply revisions
	 * @uses ideaboard_get_reply_raw_revision_log() To get the raw revision log
	 * @uses ideaboard_get_reply_author_display_name() To get the reply author
	 ...
	 * @uses apply_filters() Calls 'ideaboard_get_reply_revision_log' with the
	 *                        log and reply id
	 * @return string Revision log of the reply
	 */
	function ideaboard_get_reply_revision_log( $reply_id = 0 ) {

		// Create necessary variables
		$reply_id = ideaboard_get_reply_id( $reply_id );

		// Show the topic reply log if this is a topic in a reply loop
		if ( ideaboard_is_topic( $reply_id ) ) {
			return ideaboard_get_topic_revision_log( $reply_id );
		}

		// Get the reply revision log (out of post meta
		$revision_log = ideaboard_get_reply_raw_revision_log( $reply_id );
		...
		$revisions = ideaboard_get_reply_revisions( $reply_id );
		if ( empty( $revisions ) )
			return false;
		...
		return apply_filters( 'ideaboard_get_reply_revision_log', $r, $reply_id );
	}
		/**
		 ...
		 * @uses apply_filters() Calls 'ideaboard_get_reply_raw_revision_log'
		 *                        with the log and reply id
		 * @return string Raw revision log of the reply
		 */
		function ideaboard_get_reply_raw_revision_log( $reply_id = 0 ) {
			$reply_id     = ideaboard_get_reply_id( $reply_id );
			$revision_log = get_post_meta( $reply_id, '_ideaboard_revision_log', true );
			$revision_log = empty( $revision_log ) ? array() : $revision_log;

			return apply_filters( 'ideaboard_get_reply_raw_revision_log', $revision_log, $reply_id );
		}
 ...
 * @uses apply_filters() Calls 'ideaboard_get_reply_revisions'
 *                        with the revisions and reply id
 * @return string reply revisions
 */
function ideaboard_get_reply_revisions( $reply_id = 0 ) {
	$reply_id  = ideaboard_get_reply_id( $reply_id );
	$revisions = wp_get_post_revisions( $reply_id, array( 'order' => 'ASC' ) );

	return apply_filters( 'ideaboard_get_reply_revisions', $revisions, $reply_id );
}
 ...
 * @uses ideaboard_get_reply_revisions() To get the reply revisions
 * @uses apply_filters() Calls 'ideaboard_get_reply_revision_count'
 *                        with the revision count and reply id
 * @return string reply revision count
 */
function ideaboard_get_reply_revision_count( $reply_id = 0, $integer = false ) {
	$count  = (int) count( ideaboard_get_reply_revisions( $reply_id ) );
	$filter = ( true === $integer ) ? 'ideaboard_get_reply_revision_count_int' : 'ideaboard_get_reply_revision_count';
 ...
 *
 * @param int $reply_id Optional. Reply id
 * @uses ideaboard_get_reply_status() To get the reply status
 */
function ideaboard_reply_status( $reply_id = 0 ) {
	echo ideaboard_get_reply_status( $reply_id );
}
	/**
	 ...
	 * @uses apply_filters() Calls 'ideaboard_get_reply_status' with the reply id
	 * @return string Status of reply
	 */
	function ideaboard_get_reply_status( $reply_id = 0 ) {
		$reply_id = ideaboard_get_reply_id( $reply_id );
		return apply_filters( 'ideaboard_get_reply_status', get_post_status( $reply_id ), $reply_id );
	}
 ...
 */
function ideaboard_is_reply_published( $reply_id = 0 ) {
	$reply_status = ideaboard_get_reply_status( ideaboard_get_reply_id( $reply_id ) ) === ideaboard_get_public_status_id();
	return (bool) apply_filters( 'ideaboard_is_reply_published', (bool) $reply_status, $reply_id );
}
 ...
 */
function ideaboard_is_reply_spam( $reply_id = 0 ) {
	$reply_status = ideaboard_get_reply_status( ideaboard_get_reply_id( $reply_id ) ) === ideaboard_get_spam_status_id();
	return (bool) apply_filters( 'ideaboard_is_reply_spam', (bool) $reply_status, $reply_id );
}
 ...
 * @return bool True if spam, false if not.
 */
function ideaboard_is_reply_trash( $reply_id = 0 ) {
	$reply_status = ideaboard_get_reply_status( ideaboard_get_reply_id( $reply_id ) ) === ideaboard_get_trash_status_id();
	return (bool) apply_filters( 'ideaboard_is_reply_trash', (bool) $reply_status, $reply_id );
}
 ...
 * @param int $reply_id Optional. Reply id
 * @uses ideaboard_get_reply_id() To get the reply id
 * @uses ideaboard_get_reply_author_id() To get the reply author id
 ...
 */
function ideaboard_is_reply_anonymous( $reply_id = 0 ) {
	$reply_id = ideaboard_get_reply_id( $reply_id );
	$retval   = false;

	if ( !ideaboard_get_reply_author_id( $reply_id ) )
		$retval = true;

	elseif ( get_post_meta( $reply_id, '_ideaboard_anonymous_name', true ) )
		$retval = true;

	elseif ( get_post_meta( $reply_id, '_ideaboard_anonymous_email', true ) )
		$retval = true;

	return (bool) apply_filters( 'ideaboard_is_reply_anonymous', $retval, $reply_id );
}

/**
 * Deprecated. Use ideaboard_reply_author_display_name() instead.
 *
 ...
 *
 * @param int $reply_id Optional. Reply id
 * @uses ideaboard_get_reply_author() To get the reply author
 */
function ideaboard_reply_author( $reply_id = 0 ) {
	echo ideaboard_get_reply_author( $reply_id );
}
	/**
	 * Deprecated. Use ideaboard_get_reply_author_display_name() instead.
	 *
	 ...
	 * @param int $reply_id Optional. Reply id
	 * @uses ideaboard_get_reply_id() To get the reply id
	 * @uses ideaboard_is_reply_anonymous() To check if the reply is by an
	 *                                 anonymous user
	 * @uses get_the_author_meta() To get the reply author display name
	 * @uses get_post_meta() To get the anonymous poster name
	 * @uses apply_filters() Calls 'ideaboard_get_reply_author' with the reply
	 *                        author and reply id
	 * @return string Author of reply
	 */
	function ideaboard_get_reply_author( $reply_id = 0 ) {
		$reply_id = ideaboard_get_reply_id( $reply_id );

		if ( !ideaboard_is_reply_anonymous( $reply_id ) ) {
			$author = get_the_author_meta( 'display_name', ideaboard_get_reply_author_id( $reply_id ) );
		} else {
			$author = get_post_meta( $reply_id, '_ideaboard_anonymous_name', true );
		}

		return apply_filters( 'ideaboard_get_reply_author', $author, $reply_id );
	}
 ...
 * @uses ideaboard_get_reply_author_id() To get the reply author id
 */
function ideaboard_reply_author_id( $reply_id = 0 ) {
	echo ideaboard_get_reply_author_id( $reply_id );
}
	/**
	 ...
	 * @uses apply_filters() Calls 'ideaboard_get_reply_author_id' with the author
	 *                        id and reply id
	 * @return string Author id of reply
	 */
	function ideaboard_get_reply_author_id( $reply_id = 0 ) {
		$reply_id  = ideaboard_get_reply_id( $reply_id );
		$author_id = get_post_field( 'post_author', $reply_id );

		return (int) apply_filters( 'ideaboard_get_reply_author_id', $author_id, $reply_id );
	}
 ...
 *
 * @param int $reply_id Optional. Reply id
 * @uses ideaboard_get_reply_author_display_name()
 */
function ideaboard_reply_author_display_name( $reply_id = 0 ) {
	echo ideaboard_get_reply_author_display_name( $reply_id );
}
	/**
	 ...
	 * @uses apply_filters() Calls 'ideaboard_get_reply_author_display_name' with
	 *                        the author display name and reply id
	 * @return string Reply's author's display name
	 */
	function ideaboard_get_reply_author_display_name( $reply_id = 0 ) {
		$reply_id = ideaboard_get_reply_id( $reply_id );
		...
		// User does not have an account
			...
			$author_name = get_post_meta( $reply_id, '_ideaboard_anonymous_name', true );
		}
		...
		return apply_filters( 'ideaboard_get_reply_author_display_name', $author_name, $reply_id );
	}
 ...
 * @uses ideaboard_get_reply_author_avatar() To get the reply author id
 */
function ideaboard_reply_author_avatar( $reply_id = 0, $size = 40 ) {
	echo ideaboard_get_reply_author_avatar( $reply_id, $size );
}
	/**
	 ...
	 * @uses apply_filters() Calls 'ideaboard_get_reply_author_avatar' with the
	 *                        author avatar, reply id and size
	 * @return string Avatar of author of the reply
	 */
	function ideaboard_get_reply_author_avatar( $reply_id = 0, $size = 40 ) {
		$reply_id = ideaboard_get_reply_id( $reply_id );
		if ( !empty( $reply_id ) ) {
			// Check for anonymous user
			if ( !ideaboard_is_reply_anonymous( $reply_id ) ) {
				$author_avatar = get_avatar( ideaboard_get_reply_author_id( $reply_id ), $size );
			} else {
				$author_avatar = get_avatar( get_post_meta( $reply_id, '_ideaboard_anonymous_email', true ), $size );
			}
		} else {
			$author_avatar = '';
		}

		return apply_filters( 'ideaboard_get_reply_author_avatar', $author_avatar, $reply_id, $size );
	}
 ...
 * @uses ideaboard_get_reply_author_link() To get the reply author link
 */
function ideaboard_reply_author_link( $args = '' ) {
	echo ideaboard_get_reply_author_link( $args );
}
	/**
	 ...
	 * @uses ideaboard_is_reply_anonymous() To check if the reply is by an
	 *                                 anonymous user
	 * @uses ideaboard_get_reply_author_url() To get the reply author url
	 * @uses ideaboard_get_reply_author_avatar() To get the reply author avatar
	 * @uses ideaboard_get_reply_author_display_name() To get the reply author display
	 *                                      name
	 * @uses ideaboard_get_user_display_role() To get the reply author display role
	 * @uses ideaboard_get_reply_author_id() To get the reply author id
	 * @uses apply_filters() Calls 'ideaboard_get_reply_author_link' with the
	 *                        author link and args
	 * @return string Author link of reply
	 */
	function ideaboard_get_reply_author_link( $args = '' ) {
		...
		// Used as reply_id
		if ( is_numeric( $args ) ) {
			$reply_id = ideaboard_get_reply_id( $args );
		} else {
			$reply_id = ideaboard_get_reply_id( $r['post_id'] );
		}
		...
		if ( !empty( $reply_id ) ) {
			...
			$author_url = ideaboard_get_reply_author_url( $reply_id );
			$anonymous  = ideaboard_is_reply_anonymous( $reply_id );

			// Tweak link title if empty
			if ( empty( $r['link_title'] ) ) {
				$link_title = sprintf( empty( $anonymous ) ? __( 'View %s\'s profile', 'ideaboard' ) : __( 'Visit %s\'s website', 'ideaboard' ), ideaboard_get_reply_author_display_name( $reply_id ) );
			...
			if ( 'avatar' === $r['type'] || 'both' === $r['type'] ) {
				$author_links['avatar'] = ideaboard_get_reply_author_avatar( $reply_id, $r['size'] );
			}

			// Get display name
			if ( 'name' === $r['type']   || 'both' === $r['type'] ) {
				$author_links['name'] = ideaboard_get_reply_author_display_name( $reply_id );
			}
			...
			if ( empty( $anonymous ) && ideaboard_user_has_profile( ideaboard_get_reply_author_id( $reply_id ) ) ) {
				...
				if ( true === $r['show_role'] ) {
					$author_link[] = ideaboard_get_reply_author_role( array( 'reply_id' => $reply_id ) );
				}
		...
		return apply_filters( 'ideaboard_get_reply_author_link', $author_link, $r );
	}
 ...
 * @uses ideaboard_get_reply_author_url() To get the reply author url
 */
function ideaboard_reply_author_url( $reply_id = 0 ) {
	echo esc_url( ideaboard_get_reply_author_url( $reply_id ) );
}
	/**
	 ...
	 * @param int $reply_id Optional. Reply id
	 * @uses ideaboard_get_reply_id() To get the reply id
	 * @uses ideaboard_is_reply_anonymous() To check if the reply is by an anonymous
	 *                                 user
	 * @uses ideaboard_user_has_profile() To check if the user has a profile
	 * @uses ideaboard_get_reply_author_id() To get the reply author id
	 * @uses ideaboard_get_user_profile_url() To get the user profile url
	 * @uses get_post_meta() To get the anonymous poster's website url
	 * @uses apply_filters() Calls ideaboard_get_reply_author_url with the author
	 *                        url & reply id
	 * @return string Author URL of the reply
	 */
	function ideaboard_get_reply_author_url( $reply_id = 0 ) {
		$reply_id = ideaboard_get_reply_id( $reply_id );

		// Check for anonymous user or non-existant user
		if ( !ideaboard_is_reply_anonymous( $reply_id ) && ideaboard_user_has_profile( ideaboard_get_reply_author_id( $reply_id ) ) ) {
			$author_url = ideaboard_get_user_profile_url( ideaboard_get_reply_author_id( $reply_id ) );
		} else {
			$author_url = get_post_meta( $reply_id, '_ideaboard_anonymous_website', true );
			if ( empty( $author_url ) ) {
				$author_url = '';
			}
		}

		return apply_filters( 'ideaboard_get_reply_author_url', $author_url, $reply_id );
	}
 ...
 *
 * @param int $reply_id Optional. Reply id
 * @uses ideaboard_get_reply_author_email() To get the reply author email
 */
function ideaboard_reply_author_email( $reply_id = 0 ) {
	echo ideaboard_get_reply_author_email( $reply_id );
}
	/**
	 ...
	 * @uses apply_filters() Calls ideaboard_get_reply_author_email with the author
	 *                        email & reply id
	 * @return string Reply author email address
	 */
	function ideaboard_get_reply_author_email( $reply_id = 0 ) {
		$reply_id = ideaboard_get_reply_id( $reply_id );
		...
		if ( !ideaboard_is_reply_anonymous( $reply_id ) ) {
			...
			$user_id      = ideaboard_get_reply_author_id( $reply_id );
		...
		// Anonymous
			...
			$author_email = get_post_meta( $reply_id, '_ideaboard_anonymous_email', true );
		...
		return apply_filters( 'ideaboard_get_reply_author_email', $author_email, $reply_id );
	}
 ...
 * @uses ideaboard_get_reply_author_role() To get the reply author role
 */
function ideaboard_reply_author_role( $args = array() ) {
	echo ideaboard_get_reply_author_role( $args );
}
	/**
	 ...
	 * @uses ideaboard_get_reply_author_id() To get the reply author id
	 * @uses apply_filters() Calls ideaboard_get_reply_author_role with the author
	 *                        role & args
	 * @return string Reply author role
	 */
	function ideaboard_get_reply_author_role( $args = array() ) {
		...
		$reply_id    = ideaboard_get_reply_id( $r['reply_id'] );
		$role        = ideaboard_get_user_display_role( ideaboard_get_reply_author_id( $reply_id ) );
		$author_role = sprintf( '%1$s<div class="%2$s">%3$s</div>%4$s', $r['before'], esc_attr( $r['class'] ), esc_html( $role ), $r['after'] );

		return apply_filters( 'ideaboard_get_reply_author_role', $author_role, $r );
	}
 ...
 * @uses ideaboard_get_reply_topic_title() To get the reply topic title
 */
function ideaboard_reply_topic_title( $reply_id = 0 ) {
	echo ideaboard_get_reply_topic_title( $reply_id );
}
	/**
	 ...
	 * @uses ideaboard_get_reply_id() To get the reply id
	 * @uses ideaboard_get_reply_topic_id() To get the reply topic id
	 * @uses ideaboard_get_topic_title() To get the reply topic title
	 * @uses apply_filters() Calls 'ideaboard_get_reply_topic_title' with the
	 *                        topic title and reply id
	 * @return string Reply's topic's title
	 */
	function ideaboard_get_reply_topic_title( $reply_id = 0 ) {
		$reply_id = ideaboard_get_reply_id( $reply_id );
		$topic_id = ideaboard_get_reply_topic_id( $reply_id );

		return apply_filters( 'ideaboard_get_reply_topic_title', ideaboard_get_topic_title( $topic_id ), $reply_id );
	}
 ...
 * @uses ideaboard_get_reply_topic_id() To get the reply topic id
 */
function ideaboard_reply_topic_id( $reply_id = 0 ) {
	echo ideaboard_get_reply_topic_id( $reply_id );
}
	/**
	 ...
	 * @uses ideaboard_get_topic_id() To get the topic id
	 * @uses apply_filters() Calls 'ideaboard_get_reply_topic_id' with the topic
	 *                        id and reply id
	 * @return int Reply's topic id
	 */
	function ideaboard_get_reply_topic_id( $reply_id = 0 ) {

		// Assume there is no topic id
		$topic_id = 0;

		// Check that reply_id is valid
		if ( $reply_id = ideaboard_get_reply_id( $reply_id ) )

			// Get topic_id from reply
			if ( $topic_id = get_post_meta( $reply_id, '_ideaboard_topic_id', true ) )

				// Validate the topic_id
				$topic_id = ideaboard_get_topic_id( $topic_id );

		return (int) apply_filters( 'ideaboard_get_reply_topic_id', $topic_id, $reply_id );
	}
 ...
 */
function ideaboard_reply_forum_id( $reply_id = 0 ) {
	echo ideaboard_get_reply_forum_id( $reply_id );
}
	/**
	 ...
	 * @uses apply_filters() Calls 'ideaboard_get_reply_forum_id' with the forum
	 *                        id and reply id
	 * @return int Reply's forum id
	 */
	function ideaboard_get_reply_forum_id( $reply_id = 0 ) {
		...
		if ( $reply_id = ideaboard_get_reply_id( $reply_id ) )
			...
			if ( $forum_id = get_post_meta( $reply_id, '_ideaboard_forum_id', true ) )

				// Validate the forum_id
				$forum_id = ideaboard_get_forum_id( $forum_id );

		return (int) apply_filters( 'ideaboard_get_reply_forum_id', $forum_id, $reply_id );
	}
 ...
 *
 * @param int $reply_id Optional. Reply id
 * @uses ideaboard_get_reply_ancestor_id() To get the reply's ancestor id
 */
function ideaboard_reply_ancestor_id( $reply_id = 0 ) {
	echo ideaboard_get_reply_ancestor_id( $reply_id );
}
	/**
	 ...
	 * @uses ideaboard_get_reply_id() To get the reply id
	 */
	function ideaboard_get_reply_ancestor_id( $reply_id = 0 ) {
		...
		while ( $parent_id = ideaboard_get_reply_to( $ancestor_id ) ) {
			if ( empty( $parent_id ) || ( $parent_id === $ancestor_id ) || ( ideaboard_get_reply_topic_id( $reply_id ) === $parent_id ) || ( $parent_id === $reply_id ) ) {
				break;
			}
 ...
 */
function ideaboard_reply_to( $reply_id = 0 ) {
	echo ideaboard_get_reply_to( $reply_id );
}
	/**
	 ...
	 * @uses apply_filters() Calls 'ideaboard_get_reply_to' with the reply to id and
	 *                        reply id
	 * @return int Reply's reply to id
	 */
	function ideaboard_get_reply_to( $reply_id = 0 ) {
		...
		// Get reply_to value
		if ( !empty( $reply_id ) ) {
			$reply_to = (int) get_post_meta( $reply_id, '_ideaboard_reply_to', true );
		}

		return (int) apply_filters( 'ideaboard_get_reply_to', $reply_to, $reply_id );
	}
 ...
 */
function ideaboard_reply_to_link( $args = array() ) {
	echo ideaboard_get_reply_to_link( $args );
}
	 ...
	 * @uses apply_filters() Calls 'ideaboard_get_reply_to_link' with the formatted link,
	 *                        the arguments array, and the reply
	 * @return string Link for a reply to a reply
	 */
	function ideaboard_get_reply_to_link( $args = array() ) {
		...
		$r = ideaboard_parse_args( $args, array(
			...
			'depth'        => 0,
			'add_below'    => 'post',
			'respond_id'   => 'new-reply-' . ideaboard_get_topic_id(),
		), 'get_reply_to_link' );

		// Get the reply to use it's ID and post_parent
		$reply = ideaboard_get_reply( ideaboard_get_reply_id( (int) $r['id'] ) );

		// Bail if no reply or user cannot reply
		if ( empty( $reply ) || ! ideaboard_current_user_can_access_create_reply_form() )
			return;

		// Build the URI and return value
		$uri = remove_query_arg( array( 'ideaboard_reply_to' ) );
		$uri = add_query_arg( array( 'ideaboard_reply_to' => $reply->ID ) );
 ...
 * @since IdeaBoard (r4944)
 *
 * @uses ideaboard_get_cancel_reply_to_link() To get the reply cancellation link
 */
function ideaboard_cancel_reply_to_link( $text = '' ) {
	echo ideaboard_get_cancel_reply_to_link( $text );
}
	/**
	 ...
	 * @uses apply_filters() Calls 'ideaboard_get_cancel_reply_to_link' with the cancellation
	 *                        link and the cancel text
	 * @return string The cancellation link
	 */
	function ideaboard_get_cancel_reply_to_link( $text = '' ) {

		// Bail if not hierarchical or editing a reply
		if ( ! ideaboard_thread_replies() || ideaboard_is_reply_edit() ) {
			return;
		}
		...
		$reply_to = isset( $_GET['ideaboard_reply_to'] ) ? (int) $_GET['ideaboard_reply_to'] : 0;
 ...
 * @uses ideaboard_get_reply_position() To get the reply position
 */
function ideaboard_reply_position( $reply_id = 0, $topic_id = 0 ) {
	echo ideaboard_get_reply_position( $reply_id, $topic_id );
}
	/**
	 ...
	 * @param int $topic_id Optional. Topic id
	 * @uses ideaboard_get_reply_id() To get the reply id
	 * @uses ideaboard_get_reply_topic_id() Get the topic id of the reply id
	 * @uses ideaboard_get_topic_reply_count() To get the topic reply count
	 * @uses ideaboard_get_reply_post_type() To get the reply post type
	 * @uses ideaboard_get_reply_position_raw() To get calculate the reply position
	 * @uses ideaboard_update_reply_position() To update the reply position
	 * @uses ideaboard_show_lead_topic() Bump the count if lead topic is included
	 * @uses apply_filters() Calls 'ideaboard_get_reply_position' with the reply
	 *                        position, reply id and topic id
	 * @return int Reply position
	 */
	function ideaboard_get_reply_position( $reply_id = 0, $topic_id = 0 ) {
		...
		if ( empty( $reply_position ) ) {
			...
			if ( $reply_id !== $topic_id ) {
				$reply_position = ideaboard_get_reply_position_raw( $reply_id, $topic_id );

				// Update the reply position in the posts table so we'll never have
				// to hit the DB again.
				if ( !empty( $reply_position ) ) {
					ideaboard_update_reply_position( $reply_id, $reply_position );
				}
		...
		return (int) apply_filters( 'ideaboard_get_reply_position', $reply_position, $reply_id, $topic_id );
	}
 ...
 * @since IdeaBoard (r2667)
 *
 * @param array $args See {@link ideaboard_get_reply_admin_links()}
 * @uses ideaboard_get_reply_admin_links() To get the reply admin links
 */
function ideaboard_reply_admin_links( $args = array() ) {
	echo ideaboard_get_reply_admin_links( $args );
}
	/**
	 ...
	 * @uses ideaboard_is_topic() To check if it's the topic page
	 * @uses ideaboard_is_reply() To check if it's the reply page
	 * @uses ideaboard_get_reply_id() To get the reply id
	 * @uses ideaboard_get_reply_edit_link() To get the reply edit link
	 * @uses ideaboard_get_reply_trash_link() To get the reply trash link
	 * @uses ideaboard_get_reply_spam_link() To get the reply spam link
	 * @uses ideaboard_get_reply_move_link() To get the reply move link
	 * @uses ideaboard_get_topic_split_link() To get the topic split link
	 * @uses current_user_can() To check if the current user can edit or
	 *                           delete the reply
	 * @uses apply_filters() Calls 'ideaboard_get_reply_admin_links' with the
	 *                        reply admin links and args
	 * @return string Reply admin links
	 */
	function ideaboard_get_reply_admin_links( $args = array() ) {
		...
		$r['id'] = ideaboard_get_reply_id( (int) $r['id'] );

		// If post is a topic, return the topic admin links instead
		if ( ideaboard_is_topic( $r['id'] ) ) {
			return ideaboard_get_topic_admin_links( $args );
		}

		// If post is not a reply, return
		if ( !ideaboard_is_reply( $r['id'] ) ) {
			return;
		}

		// If topic is trashed, do not show admin links
		if ( ideaboard_is_topic_trash( ideaboard_get_reply_topic_id( $r['id'] ) ) ) {
			return;
		}

		// If no links were passed, default to the standard
		if ( empty( $r['links'] ) ) {
			$r['links'] = apply_filters( 'ideaboard_reply_admin_links', array(
				'edit'  => ideaboard_get_reply_edit_link ( $r ),
				'move'  => ideaboard_get_reply_move_link ( $r ),
				'split' => ideaboard_get_topic_split_link( $r ),
				'trash' => ideaboard_get_reply_trash_link( $r ),
				'spam'  => ideaboard_get_reply_spam_link ( $r ),
				'reply' => ideaboard_get_reply_to_link   ( $r )
			), $r['id'] );
		}

		// See if links need to be unset
		$reply_status = ideaboard_get_reply_status( $r['id'] );
		if ( in_array( $reply_status, array( ideaboard_get_spam_status_id(), ideaboard_get_trash_status_id() ) ) ) {

			// Spam link shouldn't be visible on trashed topics
			if ( ideaboard_get_trash_status_id() === $reply_status ) {
				unset( $r['links']['spam'] );

			// Trash link shouldn't be visible on spam topics
			} elseif ( ideaboard_get_spam_status_id() === $reply_status ) {
				unset( $r['links']['trash'] );
			}
		}
		...
		return apply_filters( 'ideaboard_get_reply_admin_links', $retval, $r, $args );
	}
 ...
 * @since IdeaBoard (r2740)
 *
 * @param mixed $args See {@link ideaboard_get_reply_edit_link()}
 * @uses ideaboard_get_reply_edit_link() To get the reply edit link
 */
function ideaboard_reply_edit_link( $args = '' ) {
	echo ideaboard_get_reply_edit_link( $args );
}
	 ...
	 *                           reply
	 * @uses ideaboard_get_reply_edit_url() To get the reply edit url
	 * @uses apply_filters() Calls 'ideaboard_get_reply_edit_link' with the reply
	 *                        edit link and args
	 * @return string Reply edit link
	 */
	function ideaboard_get_reply_edit_link( $args = '' ) {
		...
		if ( !current_user_can( 'edit_others_replies' ) ) {
			...
			if ( empty( $reply ) || !current_user_can( 'edit_reply', $reply->ID ) || ideaboard_past_edit_lock( $reply->post_date_gmt ) ) {
				return;
			}
		}

		// Get uri
		$uri = ideaboard_get_reply_edit_url( $r['id'] );
 ...
 * @uses ideaboard_get_reply_edit_url() To get the reply edit url
 */
function ideaboard_reply_edit_url( $reply_id = 0 ) {
	echo esc_url( ideaboard_get_reply_edit_url( $reply_id ) );
}
	/**
	 ...
	 * @uses ideaboard_get_reply_post_type() To get the reply post type
	 * @uses add_query_arg() To add custom args to the url
	 * @uses apply_filters() Calls 'ideaboard_get_reply_edit_url' with the edit
	 *                        url and reply id
	...
	function ideaboard_get_reply_edit_url( $reply_id = 0 ) {
		...
		$reply_link = ideaboard_remove_view_all( ideaboard_get_reply_permalink( $reply_id ) );
		...
		return apply_filters( 'ideaboard_get_reply_edit_url', $url, $reply_id );
	}
 ...
 * @since IdeaBoard (r2740)
 *
 * @param mixed $args See {@link ideaboard_get_reply_trash_link()}
 * @uses ideaboard_get_reply_trash_link() To get the reply trash link
 */
function ideaboard_reply_trash_link( $args = '' ) {
	echo ideaboard_get_reply_trash_link( $args );
}
	 ...
	 *                           reply
	 * @uses ideaboard_is_reply_trash() To check if the reply is trashed
	 * @uses ideaboard_get_reply_status() To get the reply status
	 * @uses add_query_arg() To add custom args to the url
	 * @uses wp_nonce_url() To nonce the url
	 * @uses esc_url() To escape the url
	 * @uses ideaboard_get_reply_edit_url() To get the reply edit url
	 * @uses apply_filters() Calls 'ideaboard_get_reply_trash_link' with the reply
	 *                        trash link and args
	 * @return string Reply trash link
	 */
	function ideaboard_get_reply_trash_link( $args = '' ) {
		...
		return apply_filters( 'ideaboard_get_reply_trash_link', $retval, $r );
	}
 ...
 * @since IdeaBoard (r2740)
 *
 * @param mixed $args See {@link ideaboard_get_reply_spam_link()}
 * @uses ideaboard_get_reply_spam_link() To get the reply spam link
 */
function ideaboard_reply_spam_link( $args = '' ) {
	echo ideaboard_get_reply_spam_link( $args );
}
	 ...
	 * @uses esc_url() To escape the url
	 * @uses ideaboard_get_reply_edit_url() To get the reply edit url
	 * @uses apply_filters() Calls 'ideaboard_get_reply_spam_link' with the reply
	 *                        spam link and args
	 * @return string Reply spam link
	 */
	function ideaboard_get_reply_spam_link( $args = '' ) {
 ...
 *
 * @param mixed $args See {@link ideaboard_get_reply_move_link()}
 * @uses ideaboard_get_reply_move_link() To get the reply move link
 */
function ideaboard_reply_move_link( $args = '' ) {
	echo ideaboard_get_reply_move_link( $args );
}
	 ...
	 * @uses wp_nonce_url() To nonce the url
	 * @uses esc_url() To escape the url
	 * @uses apply_filters() Calls 'ideaboard_get_reply_move_link' with the reply
	 *                        move link and args
	 * @return string Reply move link
	 */
	function ideaboard_get_reply_move_link( $args = '' ) {
		...
		$reply_id = ideaboard_get_reply_id( $r['id'] );
		$topic_id = ideaboard_get_reply_topic_id( $reply_id );
 ...
 *
 * @param mixed $args See {@link ideaboard_get_topic_split_link()}
 * @uses ideaboard_get_topic_split_link() To get the topic split link
 */
function ideaboard_topic_split_link( $args = '' ) {
	echo ideaboard_get_topic_split_link( $args );
}
	 ...
	 * @uses wp_nonce_url() To nonce the url
	 * @uses esc_url() To escape the url
	 * @uses apply_filters() Calls 'ideaboard_get_topic_split_link' with the topic
	 *                        split link and args
	 * @return string Topic split link
	 */
	function ideaboard_get_topic_split_link( $args = '' ) {
 ...
 * @uses ideaboard_get_reply_class() To get the reply class
 */
function ideaboard_reply_class( $reply_id = 0, $classes = array() ) {
	echo ideaboard_get_reply_class( $reply_id, $classes );
}
	/**
	 ...
	 * @param array Extra classes you can pass when calling this function
	 * @uses ideaboard_get_reply_id() To validate the reply id
	 * @uses ideaboard_get_reply_forum_id() To get the reply's forum id
	 * @uses ideaboard_get_reply_topic_id() To get the reply's topic id
	 * @uses get_post_class() To get all the classes including ours
	 * @uses apply_filters() Calls 'ideaboard_get_reply_class' with the classes
	...
	function ideaboard_get_reply_class( $reply_id = 0, $classes = array() ) {
		...
		$classes   = apply_filters( 'ideaboard_get_reply_class', $classes, $reply_id );
 ...
 *
 * @uses ideaboard_get_topic_pagination_count() To get the topic pagination count
 */
function ideaboard_topic_pagination_count() {
	echo ideaboard_get_topic_pagination_count();
}
	/**
	 ...
	 * @uses ideaboard_show_lead_topic() Are we showing the topic as a lead?
	 * @uses apply_filters() Calls 'ideaboard_get_topic_pagination_count' with the
	 *                        pagination count
	...
	function ideaboard_get_topic_pagination_count() {
		...
		if ( ideaboard_thread_replies() && ideaboard_is_single_topic() ) {
			...
			// Adjust for topic
			$threads--;
			$retstr  = sprintf( _n( 'Viewing %1$s reply thread', 'Viewing %1$s reply threads', $threads, 'bideaboard' ), ideaboard_number_format( $threads ) );
		...
		// Filter and return
		return apply_filters( 'ideaboard_get_topic_pagination_count', esc_html( $retstr ) );
	}
 ...
 * @since IdeaBoard (r2519)
 *
 * @uses ideaboard_get_topic_pagination_links() To get the topic pagination links
 */
function ideaboard_topic_pagination_links() {
	echo ideaboard_get_topic_pagination_links();
}
	/**
	 ...
	 * @since IdeaBoard (r2519)
	 *
	 * @uses apply_filters() Calls 'ideaboard_get_topic_pagination_links' with the
	 *                        pagination links
 ...
 *
 * @uses ideaboard_get_form_reply_content() To get value of reply content field
 */
function ideaboard_form_reply_content() {
	echo ideaboard_get_form_reply_content();
}
	/**
	 ...
	 * @uses apply_filters() Calls 'ideaboard_get_form_reply_content' with the content
	 * @return string Value of reply content field
	 */
	function ideaboard_get_form_reply_content() {

		// Get _POST data
		if ( ideaboard_is_post_request() && isset( $_POST['ideaboard_reply_content'] ) ) {
			$reply_content = stripslashes( $_POST['ideaboard_reply_content'] );

		// Get edit data
		} elseif ( ideaboard_is_reply_edit() ) {
			$reply_content = ideaboard_get_global_post_field( 'post_content', 'raw' );
		...
		return apply_filters( 'ideaboard_get_form_reply_content', $reply_content );
	}
 ...
 * @since IdeaBoard (r4944)
 *
 * @uses ideaboard_get_form_reply_to() To get value of the reply to field
 */
function ideaboard_form_reply_to() {
	echo ideaboard_get_form_reply_to();
}
	 ...
	 * @since IdeaBoard (r4944)
	 *
	 * @uses ideaboard_get_reply_id() To validate the reply to
	 * @uses apply_filters() Calls 'ideaboard_get_form_reply_to' with the reply to
	 * @return string Value of reply to field
	 */
	function ideaboard_get_form_reply_to() {
		...
		if ( empty( $reply_to ) ) {
			$reply_to = ideaboard_get_reply_to();
		}

		return (int) apply_filters( 'ideaboard_get_form_reply_to', $reply_to );
	}
 ...
 * @since IdeaBoard (r31301)
 *
 * @uses ideaboard_get_form_reply_log_edit() To get the reply log edit value
 */
function ideaboard_form_reply_log_edit() {
	echo ideaboard_get_form_reply_log_edit();
}
	/**
	 ...
	 * @since IdeaBoard (r31301)
	 *
	 * @uses apply_filters() Calls 'ideaboard_get_form_reply_log_edit' with the
	 *                        log edit value
	 * @return string Reply log edit checked value
	 */
	function ideaboard_get_form_reply_log_edit() {

		// Get _POST data
		if ( ideaboard_is_post_request() && isset( $_POST['ideaboard_log_reply_edit'] ) ) {
			$reply_revision = $_POST['ideaboard_log_reply_edit'];
		...
		return apply_filters( 'ideaboard_get_form_reply_log_edit', checked( $reply_revision, true, false ) );
	}
 ...
 * @since IdeaBoard (r31301)
 *
 * @uses ideaboard_get_form_reply_edit_reason() To get the reply edit reason value
 */
function ideaboard_form_reply_edit_reason() {
	echo ideaboard_get_form_reply_edit_reason();
}
	/**
	 ...
	 * @since IdeaBoard (r31301)
	 *
	 * @uses apply_filters() Calls 'ideaboard_get_form_reply_edit_reason' with the
	 *                        reply edit reason value
	 * @return string Reply edit reason value
	 */
	function ideaboard_get_form_reply_edit_reason() {

		// Get _POST data
		if ( ideaboard_is_post_request() && isset( $_POST['ideaboard_reply_edit_reason'] ) ) {
			$reply_edit_reason = $_POST['ideaboard_reply_edit_reason'];
		...
		return apply_filters( 'ideaboard_get_form_reply_edit_reason', esc_attr( $reply_edit_reason ) );
	}
