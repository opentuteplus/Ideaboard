...
<form id="bbp-your-profile" action="<?php bbp_user_profile_edit_url( bbp_get_displayed_user_id() ); ?>" method="post" enctype="multipart/form-data">
	...
	<h2 class="entry-title"><?php bbp_is_user_home_edit() ? _e( 'About Yourself', 'ideaboard' ) : _e( 'About the user', 'ideaboard' ); ?></h2>

	<fieldset class="bbp-form">
		<legend><?php bbp_is_user_home_edit() ? _e( 'About Yourself', 'ideaboard' ) : _e( 'About the user', 'ideaboard' ); ?></legend>
		...
		<div>
			...
			if ( !empty( $new_email ) && $new_email !== bbp_get_displayed_user_field( 'user_email', 'edit' ) ) : ?>
				...
				<span class="updated inline">
					...
					<?php printf( __( 'There is a pending email address change to <code>%1$s</code>. <a href="%2$s">Cancel</a>', 'ideaboard' ), $new_email['newemail'], esc_url( self_admin_url( 'user.php?dismiss=' . bbp_get_current_user_id()  . '_new_email' ) ) ); ?>
	...
	<fieldset class="submit">
		...
		<div>
			...
			<button type="submit" tabindex="<?php bbp_tab_index(); ?>" id="bbp_user_edit_submit" name="bbp_user_edit_submit" class="button submit user-submit"><?php bbp_is_user_home_edit() ? _e( 'Update Profile', 'ideaboard' ) : _e( 'Update User', 'ideaboard' ); ?></button>
		</div>
	</fieldset>
