		...
		<div id="move-reply-<?php bbp_topic_id(); ?>" class="bbp-reply-move">
			...
			<form id="move_reply" name="move_reply" method="post" action="<?php the_permalink(); ?>">
				...
				<fieldset class="bbp-form">
					...
					<legend><?php printf( __( 'Move reply "%s"', 'ideaboard' ), bbp_get_reply_title() ); ?></legend>
					...
					<div>
						...
						<fieldset class="bbp-form">
							...
							<div>
								...
								<label for="bbp_reply_move_option_reply"><?php printf( __( 'New topic in <strong>%s</strong> titled:', 'ideaboard' ), bbp_get_forum_title( bbp_get_reply_forum_id( bbp_get_reply_id() ) ) ); ?></label>
								<input type="text" id="bbp_reply_move_destination_title" value="<?php printf( __( 'Moved: %s', 'ideaboard' ), bbp_get_reply_title() ); ?>" tabindex="<?php bbp_tab_index(); ?>" size="35" name="bbp_reply_move_destination_title" />
							</div>
							...
							<?php if ( bbp_has_topics( array( 'show_stickies' => false, 'post_parent' => bbp_get_reply_forum_id( bbp_get_reply_id() ), 'post__not_in' => array( bbp_get_reply_topic_id( bbp_get_reply_id() ) ) ) ) ) : ?>
								...
								<div>
									...
									<label for="bbp_reply_move_option_existing"><?php _e( 'Use an existing topic in this forum:', 'ideaboard' ); ?></label>
						...
						<div class="bbp-submit-wrapper">
							<button type="submit" tabindex="<?php bbp_tab_index(); ?>" id="bbp_move_reply_submit" name="bbp_move_reply_submit" class="button submit"><?php _e( 'Submit', 'ideaboard' ); ?></button>
						</div>
					</div>
